        ...
        while (1)
        {
            ...
            if (ret == ESP_OK)
            {
                ...
                for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES)
                {
                    ...
                    {
                        ESP_LOGW(TAG,
                                 "Invalid data [%s_%" PRIu32 "_%" PRIx32 "]",
                                 unit, chan_num, data);
                    }
                }
                /**
                 * Because printing is slow, so every time you call
                 * `ulTaskNotifyTake`, it will immediately return. To avoid a
                 * task watchdog timeout, add a delay here. When you replace
                 * the way you process the data, usually you don't need this
                 * delay (as this task will block for a while).
                 */
                vTaskDelay(100);
            }
