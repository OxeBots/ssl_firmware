        ...
        if (usElapsed >= _samplePeriodUs && !eStopAbort) {
          *_output = _outputStart;
          ...
          if (_serialMode == printALL || _serialMode == printDEBUG) {
            printf(" sec: %.4f", (float)((_settlePeriodUs - settleElapsed) * 0.000001f));
            printf("  out: %.2f", *_output);
            printf("  pv: %.3f", pvInst);
            printf("  settling  ⤳⤳\n");
          }
          _tunerStatus = sample;
...
void sTune::printTestRun() {
  ...
  if (sampleCount < _samples) {
    if (_serialMode == printALL || _serialMode == printDEBUG) {
      printf(" sec: %.4f", us * 0.000001f);
      printf("  out: %.2f", *_output);
      printf("  pv: %.3f", pvInst);
      if (_serialMode == printDEBUG && (_action == direct5T || _action == reverse5T)) {
        printf("  pvPk: %.3f", pvPk);
        printf("  pvPkCount: %d", pvPkCount);
        printf("  ipCount: %d", ipCount);
      }
      if (_serialMode == printDEBUG && (_action == directIP || _action == reverseIP)) {
        printf("  ipCount: %d", ipCount);
      }
      printf("  tan: %.3f", pvTangent);
      if (pvInst > 0.9f * eStop)                        printf(" ⚠");
      if (pvTangent - pvTangentPrev > 0 + epsilon)      printf(" ↗\n");
      else if (pvTangent - pvTangentPrev < 0 - epsilon) printf(" ↘\n");
      else                                              printf(" →\n");
    }
  }
}
...
void sTune::printTunings() {
  printf(" Tuning Method: ");
  if (_tuningMethod == ZN_PID) printf("ZN_PID\n");
  else if (_tuningMethod == DampedOsc_PID) printf("Damped_PID\n");
  else if (_tuningMethod == NoOvershoot_PID) printf("NoOvershoot_PID\n");
  else if (_tuningMethod == CohenCoon_PID) printf("CohenCoon_PID\n");
  else if (_tuningMethod == Mixed_PID) printf("Mixed_PID\n");
  else if (_tuningMethod == ZN_PI) printf("ZN_PI\n");
  else if (_tuningMethod == DampedOsc_PI) printf("Damped_PI\n");
  else if (_tuningMethod == NoOvershoot_PI) printf("NoOvershoot_PI\n");
  else if (_tuningMethod == CohenCoon_PI) printf("CohenCoon_PI\n");
  else printf("Mixed_PI\n");
  printf("  Kp: %.3f\n", sTune::GetKp());
  printf("  Ki: %.3f  Ti: %.3f\n", sTune::GetKi(), sTune::GetTi());
  printf("  Kd: %.3f  Td: %.3f\n\n", sTune::GetKd(), sTune::GetTd());
}

void sTune::printResults() {
  if (_serialMode == printALL || _serialMode == printDEBUG || _serialMode == printSUMMARY) {
    printf("\n Controller Action: ");
    if (_action == directIP) printf("directIP\n");
    else if (_action == direct5T) printf("direct5T\n");
    else if (_action == reverseIP) printf("reverseIP\n");
    else printf("reverse5T\n");
    printf("\n Output Start:      %.2f\n", _outputStart);
    printf(" Output Step:       %.2f\n", _outputStep);
    printf(" Sample Sec:        %.4f\n\n", _samplePeriodUs * 0.000001f);
    if (_serialMode == printDEBUG && (_action == directIP || _action == reverseIP)) {
      printf(" Ip Sec:            %.4f\n", ipUs * 0.000001f);
      printf(" Ip Slope:          %.3f", slopeIp);
      if (_action == directIP || _action == direct5T) printf(" ↑\n");
      else  printf(" ↓\n");
      printf(" Ip Pv:             %.3f\n", pvIp);
    }
    printf(" Pv Start:          %.3f\n", pvStart);
    if (_action == directIP || _action == direct5T) printf(" Pv Max:            %.3f\n", pvMax);
    else printf(" Pv Min:            %.3f\n", pvMax);
    printf(" Pv Diff:           %.3f\n\n", pvMax - pvStart);
    printf(" Process Gain:      %.3f\n", _Ku);
    printf(" Dead Time Sec:     %.3f\n", _td);
    printf(" Tau Sec:           %.3f\n\n", _Tu);

    float controllability = _Tu / _td + epsilon;
    if (controllability > 99.9) controllability = 99.9;
    printf(" Tau/Dead Time:     %.1f", controllability);
    if (controllability > 0.75) printf(" (easy to control)\n");
    else if (controllability > 0.25) printf(" (average controllability)\n");
    else printf(" (difficult to control)\n");

    float sampleTimeCheck = _Tu / (_samplePeriodUs * 0.000001f);
    printf(" Tau/Sample Period: %.1f", sampleTimeCheck);
    if (sampleTimeCheck >= 10) printf(" (good sample rate)\n\n");
    else printf(" (low sample rate)\n\n");
    sTune::printTunings();
    sampleCount++;
  }
}
