 ...
 *
 * To run it just set the correct pin numbers for the BLDC driver and sensor
 * CPR value and chip select pin.
 *
 * The program will rotate your motor a specific amount and check how much it
 * moved, and by doing a simple calculation calculate your pole pair number.
 * The pole pair number will be outputted to the serial terminal.
 *
 * If the pole pair number is well estimated your motor will start to spin in
 * voltage mode with 2V target.
 *
 * If the code calculates negative pole pair number please invert your motor
 * connector.
 *
...
void setup()
{
    ...
    if (pp <= 0)
    {
        ...
        Serial.println(
          F(" - Try changing the search_voltage value or motor/sensor "
            "configuration."));
        return;
    }
    else if (pp > 30)
    ...
    {
        Serial.println(
          F("If PP is estimated well your motor should turn now!"));
        Serial.println(
          F(" - If it is not moving try to relaunch the program!"));
        Serial.println(
          F(" - You can also try to adjust the target voltage using serial "
            "terminal!"));
    }
...
// utility function enabling serial communication with the user to set the
// target values this function can be implemented in serialEvent function as
// well
